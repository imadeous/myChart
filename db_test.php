...
try {
    ...
    $stmt = $pdo->query("SELECT DATE_FORMAT(created_at, '%Y-%m') as month, type, SUM(amount) as amount FROM transactions GROUP BY month, type ORDER BY month, type LIMIT 10");
    $data = $stmt->fetchAll();
